...
## Configuration
Update thresholds, default values, and UI settings in `config.py`. If you replace the model artifacts, ensure the feature schema matches the input DataFrame generated in `utils.py`.

## Model & Data Notes
The app loads `pipeline.pkl` and `shap_explainer.pkl` from the project root. If these files are missing or replaced, the app will surface a startup error or incorrect interpretations.

## Disclaimer
This project is for educational and research purposes only and is not a substitute for professional medical advice, diagnosis, or treatment.
